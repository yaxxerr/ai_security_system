...
export default function Landing() {
  ...
  // Header components
  const headerStart = (
    <div className="flex align-items-center gap-3 cursor-pointer" onClick={() => navigate('/') }>
      <img src="/logo.svg" alt="AI Security System" className="w-1 h-1" />
      <h1 className="text-xl font-bold m-0">AI Security System</h1>
    </div>
  );

  const headerEnd = (
    <div className="flex align-items-center gap-3">
      <Button icon="pi pi-home" className="p-button-text p-button-rounded" aria-label="Home" onClick={() => navigate('/')} />
      {user ? (
        <Button
